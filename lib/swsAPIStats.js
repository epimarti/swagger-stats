...
swsAPIStats.prototype.initializePromClient = function (swsOptions) {
    var metricsConfig = swsUtil.swsMetrics(swsOptions)
    var labelNames = this.traceCallByUser
        ? ['method', 'path', 'code', 'user']
        : ['method', 'path', 'code']

    if (!this.promClientMetrics.api_request_total)
        this.promClientMetrics.api_request_total = new promClient.Counter({
            name: metricsConfig.api_request_total.name,
            help: metricsConfig.api_request_total.help,
            labelNames
        });

    if (!this.promClientMetrics.api_request_duration_milliseconds)
        this.promClientMetrics.api_request_duration_milliseconds = new promClient.Histogram({
            name: metricsConfig.api_request_duration_milliseconds.name,
            help: metricsConfig.api_request_duration_milliseconds.help,
            labelNames,
            buckets: this.durationBuckets
        });

    if (!this.promClientMetrics.api_request_size_bytes)
        this.promClientMetrics.api_request_size_bytes = new promClient.Histogram({
            name: metricsConfig.api_request_size_bytes.name,
            help: metricsConfig.api_request_size_bytes.help,
            labelNames,
            buckets: this.requestSizeBuckets
        });

    if (!this.promClientMetrics.api_response_size_bytes)
        this.promClientMetrics.api_response_size_bytes = new promClient.Histogram({
            name: metricsConfig.api_response_size_bytes.name,
            help: metricsConfig.api_response_size_bytes.help,
            labelNames,
...
swsAPIStats.prototype.initialize = function (swsOptions) {
    ...
    this.traceCallByUser = this.options && this.options.prometheusTraceCallByUser
    this.initializePromClient(this.options)
...
swsAPIStats.prototype.countResponse = function (res) {
    ...
    if (this.traceCallByUser) {
        labels['user'] = this.options.getUserName(req)
    }
    this.promClientMetrics.api_request_total.inc(labels);

    this.promClientMetrics.api_request_duration_milliseconds.observe(labels, req.sws.duration);
    this.promClientMetrics.api_request_size_bytes.observe(labels, req.sws.req_clength);
    this.promClientMetrics.api_response_size_bytes.observe(labels, req.sws.res_clength);
};
